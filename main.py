...
car_data = []
empty_page_count = 0  # Variable to track the number of consecutive empty pages
max_empty_pages = 10  # Stop after 10 consecutive empty pages
page_counter = 0  # Counter to track the number of successfully scraped pages

for count in range(1, 4001):  # You can set the range up to 4000 pages
    ...
    try:
        ...
        if not car_cards:  # If no car cards are found, increment the empty page counter
            empty_page_count += 1
            if empty_page_count >= max_empty_pages:  # Stop the scraper if 10 consecutive empty pages
                print("No more pages with cars. Stopping the scraper.")
                break
        else:
            empty_page_count = 0  # Reset the counter if cars are found
            page_counter += 1  # Increment the page counter
            # Print the page counter
            print(f"Page {page_counter} scraped successfully.")
            ...
            for card in car_cards:
                ...
                if base_info:
                    ...
                    if vin_span:
                        ...
                        vin_code = vin_inner[0].text.strip(
                        ) if vin_inner else None
                ...
                data_add = date_span.get(
                    'data-add-date') if date_span else None
                ...
                sold_date = None
                if footer:
                    sold_span = footer.find(
                        'span', attrs={'data-sold-date': True})
                    if sold_span:
                        sale_status = "sold"
                ...
                car_info = {
                    ...
                    "sold_date": sold_date,
                    "top_level": top,
                    "accident_status": accident_status  # <-- added field
                }
